 ...
 * Description: Provides both widgets and shortcodes to help you display Treehouse stuff
 * Version: 1.0
 * Author: Zac Gordon
 * Author URI: http://wp.zacgordon.com/wptreehouse-badges-plugin
 * License: GPL2
 *
*/
...
/*Now tying to a proper hook with an action hook*/
...
function wptreehouse_badges_options_page(){
	...
	if( !current_user_can('manage_options') ){
		...
		wp_die('You do not have sufficient perms');
...
class Wptreehouse_Badges_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		// Widget output
		// Don't forget to update the form, the update, and finally the widget function
	...
	function form( $instance ) {
		...
		// Necessary because we need to display profile information in the instance
		$options = get_option( 'wptreehouse_badges' );
		$wptreehouse_profile = $options['wptreehouse_profile'];
...
function wptreehouse_badges_shortcode( $atts, $content = null ){
	...
	$options = get_option( 'wp_treehouse_badges' );
	$wptreehouse_profile = $options['wptreehouse_profile'];
	
	ob_start();
	
	require( 'inc/front-end.php' );
	//working with shortcodes and using require statements, you need to add additional code, thus you use the buffer which will hold the require file in temporary storage until it is needed
...
function wptreehouse_badges_refresh_profile(){ //this is an AJAX call that will refresh the profile automatically
	...
	if( $update_difference > 86400 ){ //update statement to update the feed if difference is greater than 24 hours
		$wptreehouse_username = $options['wptreehouse_username'];
	...
	die(); //when using AJAX, let's AJAX know when the AJAX function is completed

}
add_action( 'wp_ajax_wptreehouse_badges_refresh_profile', 'wptreehouse_badges_refresh_profile' ); //custom hook for the AJAX function
